...
class StatusWiseSalesReportWizard(models.TransientModel):
    _name = 'status.wise.sales.report.wizard'
    _description = 'Status Wise Sales Report Wizard'
    ...
    partner_ids = fields.Many2many('res.partner', 'status_sale_register_partner', 'status_sale_wizard_id', 'res_partner_id', string='Customer')
    user_ids = fields.Many2many('res.users', 'status_sale_register_user', 'status_sale_wizard_id', 'res_users_id', string='Sales Manager')
    currency_ids = fields.Many2many('res.currency', 'status_sale_register_currency', 'status_sale_wizard_id', 'res_currency_id', string='Currency')
    team_ids = fields.Many2many('crm.team', 'status_sale_register_team', 'status_sale_wizard_id', 'team_id', string='Sales Team')
    warehouse_ids = fields.Many2many('stock.warehouse', 'status_sale_register_warehouse', 'status_sale_wizard_id', 'warehouse_id', string='Warehouse')
    name = fields.Char(string='File Name', readonly=True)
    output = fields.Binary(string='format', readonly=True)
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('status.wise.sales.report.wizard'))
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(StatusWiseSalesReportWizard, self).fields_view_get(
    ...
    def action_report(self):
        ...
        # print(">>>>>>>>>>>>>date_from>>>>>>>1111>>>>>>>>>", date_from, type(date_from))
        ...
        sheet1.write_merge(rc, rc, 0, 23, (self.company_id.name), Style.title_ice_blue())
        sheet1.write_merge(r1, r1, 0, 23, title, Style.title_ice_blue())
        ...
        amount_untaxed_cc_subtotal = 0.00
        discount_amount_cc_subtotal = 0.00
        tax_amount_cc_subtotal = 0.00
        total_amount_cc_subtotal = 0.00
        subtotal_row = 0
        row = 3
        s_no = 0
        ...
        for each in t:
            ...
            if so_browse.currency_id and so_browse.company_id and so_browse.currency_id != so_browse.company_id.currency_id:
                print('>>>>>>>1111>>>>>>>>>>',so_browse.currency_id)
                print('>>>>>>>222>>>>>>>>>>',so_browse.company_id)
                print('>>>>>>>>333>>>>>>>>>',so_browse.company_id.currency_id)
        ...
        sheet1.write_merge(subtotal_row, subtotal_row, 22, 23, " ", Style.normal_right_ice_blue())
        stream = BytesIO()
        wbk.save(stream)
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'status.wise.sales.report.wizard',
            'res_id': self.id,
