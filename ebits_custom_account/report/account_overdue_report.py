...
class ReportOverdue(models.AbstractModel):
    ...
    def _get_account_move_lines(self, partner_ids):
        ...
        self.env.cr.execute("SELECT m.name AS move_id, to_char(l.date, 'dd-mm-yyyy') as date, l.name, l.ref, to_char(l.date_maturity, 'dd-mm-yyyy') as date_maturity, l.partner_id, l.blocked, l.amount_currency, l.currency_id, ap.customer_receipt as receipt_no, "
            ...
            "WHERE l.partner_id IN %s AND at.type IN ('receivable', 'payable') GROUP BY to_char(l.date, 'dd-mm-yyyy'), l.name, l.ref, l.date_maturity, to_char(l.date_maturity, 'dd-mm-yyyy'), l.partner_id, at.type, l.blocked, l.amount_currency, l.currency_id, l.move_id, m.name, ap.customer_receipt", (((fields.date.today(), ) + (tuple(partner_ids),))))
            ...
            for line_tmp in lines[partner_id]:
                ...
                if not line['blocked']:
                    ...
                    if company_currency == currency:
                        ...
                        totals[partner_id][currency]['total'] += line['debit'] - (-1 * line['credit'])
        docargs = {
            'doc_ids': docids,
