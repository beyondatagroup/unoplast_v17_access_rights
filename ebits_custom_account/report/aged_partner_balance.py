...
class AgedPartnerBalanceReportAccount(models.TransientModel):
    ...
    cumul_amount_residual_other = fields.Float(digits=(16, 2))
    cumul_current_other = fields.Float(digits=(16, 2))
    cumul_age_30_days_other = fields.Float(digits=(16, 2))
    cumul_age_60_days_other = fields.Float(digits=(16, 2))
    cumul_age_90_days_other = fields.Float(digits=(16, 2))
    cumul_age_120_days_other = fields.Float(digits=(16, 2))
    cumul_older_other = fields.Float(digits=(16, 2))
    ...
    percent_current_other = fields.Float(digits=(16, 2))
    percent_age_30_days_other = fields.Float(digits=(16, 2))
    percent_age_60_days_other = fields.Float(digits=(16, 2))
    percent_age_90_days_other = fields.Float(digits=(16, 2))
    percent_age_120_days_other = fields.Float(digits=(16, 2))
    percent_older_other = fields.Float(digits=(16, 2))
...
class AgedPartnerBalanceReportLine(models.TransientModel):
    ...
    amount_residual_other = fields.Float(digits=(16, 2))
    current_other = fields.Float(digits=(16, 2))
    age_30_days_other = fields.Float(digits=(16, 2))
    age_60_days_other = fields.Float(digits=(16, 2))
    age_90_days_other = fields.Float(digits=(16, 2))
    age_120_days_other = fields.Float(digits=(16, 2))
    older_other = fields.Float(digits=(16, 2))
...
class AgedPartnerBalanceReportCompute(models.TransientModel):
    ...
    @api.multi
    def print_report(self, other_currency=False, xlsx_report=False):
        self.ensure_one()
        self.compute_data_for_report()
        if other_currency:
            if xlsx_report:
                report_name = 'ebits_custom_account.report_aged_partner_balance_other_xlsx'
            else:
                report_name = 'ebits_custom_account.report_aged_partner_balance_other_qweb'
        else:
            if xlsx_report:
                report_name = 'ebits_custom_account.report_aged_partner_balance_xlsx'
            else:
                report_name = 'ebits_custom_account.report_aged_partner_balance_qweb'
        return self.env['report'].with_context(landscape=True).get_action(docids=self.ids, report_name=report_name)
...
WITH
    date_range AS
        (
            SELECT
                DATE %s + INTEGER '1' AS date_current_1,
                DATE %s - INTEGER '0' AS date_current,
                DATE %s - INTEGER '30' AS date_less_30_days,
                DATE %s - INTEGER '31' AS date_less_31_days,
                DATE %s - INTEGER '61' AS date_less_61_days,
                DATE %s - INTEGER '62' AS date_less_62_days,
                DATE %s - INTEGER '92' AS date_less_92_days,
                DATE %s - INTEGER '93' AS date_less_93_days,
                DATE %s - INTEGER '123' AS date_less_123_days,
                DATE %s - INTEGER '124' AS date_older_124
        )
INSERT INTO
...
SELECT
    ...
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_current_1
    ...
    SUM(
        CASE
            WHEN 
                rlo.date_due <= date_range.date_current
    ...
    SUM(
        CASE
            WHEN
                rlo.date_due <= date_range.date_less_31_days
                AND rlo.date_due >= date_range.date_less_61_days
            THEN rlo.amount_residual
        END
    ) AS age_60_days,
    SUM(
        CASE
            WHEN
                rlo.date_due <= date_range.date_less_62_days
                AND rlo.date_due >= date_range.date_less_92_days
            THEN rlo.amount_residual
        END
    ) AS age_90_days,
    SUM(
        CASE
            WHEN
                rlo.date_due <= date_range.date_less_93_days
                AND rlo.date_due >= date_range.date_less_123_days
            THEN rlo.amount_residual
        END
    ) AS age_120_days,
    SUM(
        CASE
            WHEN 
                rlo.date_due <= date_range.date_older_124
            THEN rlo.amount_residual
        END
    ) AS older,
    SUM(rlo.amount_residual_currency) AS amount_residual_other,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_current_1
            THEN rlo.amount_residual_currency
        END
    ) AS current_other,
    SUM(
        CASE
    ...
    rlo.currency_id AS currency_id,
...
INNER JOIN
    res_currency currency ON currency.id = rlo.currency_id
        """
        ...
        query_inject_line += """
            ...
            GROUP BY
                ...
                currency.symbol
                    """
        query_inject_line_params = (self.date_at,) * 10
...
SELECT
    ...
    CASE
        ...
        THEN rlo.amount_residual
    END AS older,
    rlo.amount_residual_currency AS amount_residual_other,
    CASE
        WHEN 
            rlo.date_due >= date_range.date_current_1
        THEN rlo.amount_residual_currency
    END AS current_other,
    CASE
        WHEN
            rlo.date_due <= date_range.date_current
            AND rlo.date_due >= date_range.date_less_30_days
        THEN rlo.amount_residual_currency
    END AS age_30_days_other,
    CASE
        WHEN
            rlo.date_due <= date_range.date_less_31_days
            AND rlo.date_due >= date_range.date_less_61_days
        THEN rlo.amount_residual_currency
    END AS age_60_days_other,
    CASE
        WHEN
            rlo.date_due <= date_range.date_less_62_days
            AND rlo.date_due >= date_range.date_less_92_days
        THEN rlo.amount_residual_currency
    END AS age_90_days_other,
    CASE
        WHEN
            rlo.date_due <= date_range.date_less_93_days
            AND rlo.date_due >= date_range.date_less_123_days
        THEN rlo.amount_residual_currency
    END AS age_120_days_other,
    CASE
        WHEN
            rlo.date_due <= date_range.date_older_124
        THEN rlo.amount_residual_currency
...
INNER JOIN
    res_currency currency ON currency.id = rlo.currency_id
        ...
        query_inject_move_line += """
            ...
            AND ra.report_id = %s
                    """
        query_inject_move_line_params = (self.date_at,) * 10
    ...
    cumuls AS
        (
            SELECT
                ...
                SUM(rl.amount_residual_other) AS cumul_amount_residual_other,
                SUM(rl.current_other) AS cumul_current_other,
                SUM(rl.age_30_days_other) AS cumul_age_30_days_other,
                SUM(rl.age_60_days_other) AS cumul_age_60_days_other,
                SUM(rl.age_90_days_other) AS cumul_age_90_days_other,
                SUM(rl.age_120_days_other) AS cumul_age_120_days_other,
                SUM(rl.older_other) AS cumul_older_other
            FROM
...
SET
    ...
    cumul_amount_residual_other = c.cumul_amount_residual_other,
    cumul_current_other = c.cumul_current_other,
    cumul_age_30_days_other = c.cumul_age_30_days_other,
    cumul_age_60_days_other = c.cumul_age_60_days_other,
    cumul_age_90_days_other = c.cumul_age_90_days_other,
    cumul_age_120_days_other = c.cumul_age_120_days_other,
    cumul_older_other = c.cumul_older_other,
    percent_current =
        CASE
    ...
    percent_older =
        ...
        END,
    percent_current_other =
        CASE
            WHEN c.cumul_amount_residual_other != 0
            THEN 100 * c.cumul_current_other / c.cumul_amount_residual_other
        END,
    percent_age_30_days_other =
        CASE
            WHEN c.cumul_amount_residual_other != 0
            THEN 100 * c.cumul_age_30_days_other / c.cumul_amount_residual_other
        END,
    percent_age_60_days_other =
        CASE
            WHEN c.cumul_amount_residual_other != 0
            THEN 100 * c.cumul_age_60_days_other / c.cumul_amount_residual_other
        END,
    percent_age_90_days_other =
        CASE
            WHEN c.cumul_amount_residual_other != 0
            THEN 100 * c.cumul_age_90_days_other / c.cumul_amount_residual_other
        END,
    percent_age_120_days_other =
        CASE
            WHEN c.cumul_amount_residual_other != 0
            THEN 100 * c.cumul_age_120_days_other / c.cumul_amount_residual_other
        END,
    percent_older_other =
        CASE
            WHEN c.cumul_amount_residual_other != 0
            THEN 100 * c.cumul_older_other / c.cumul_amount_residual_other
        END
FROM
    cumuls c
WHERE
