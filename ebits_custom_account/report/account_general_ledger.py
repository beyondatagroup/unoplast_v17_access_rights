...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        if init_balance:
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, COALESCE(SUM(l.amount_currency),0.0) AS amount_currency, COALESCE(SUM(l.amount_currency),0.0) AS currency_balance, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
            ...
            for row in cr.dictfetchall():
                ldate = False
                if row['ldate']:
                    ldate = datetime.strptime(row['ldate'], "%Y-%m-%d")
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.amount_currency as currency_balance, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
        ...
        for row in cr.dictfetchall():
            ...
            for line in move_lines.get(row['account_id']):
                ...
                currency_balance += line['amount_currency'] and line['amount_currency'] or 0.00
            row['balance'] += balance
            row['currency_balance'] += currency_balance
            move_lines[row.pop('account_id')].append(row)
        ...
        for account in accounts:
            ...
            if account.currency_id:
                ...
                for line in res.get('move_lines'):
                    ...
                    res['currency_balance'] += line['amount_currency']
            else:
