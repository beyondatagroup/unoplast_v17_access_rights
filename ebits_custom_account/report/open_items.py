...
class OpenItemsReportCompute(models.TransientModel):
    ...
    def print_report(self, xlsx_report=False):
        ...
        if xlsx_report:
            report_name = 'ebits_custom_account.' \
...
WITH
    accounts AS
        ...
        if self.filter_partner_ids:
            ...
            """
        if self.show_other_currency:
            other_currency_query = """ AND
                p.transaction_currency_id != """ + str(self.env.user.company_id.currency_id.id)
            query_inject_account += other_currency_query
        query_inject_account += """
            GROUP BY
                a.id
        )
...
WITH
    accounts_partners AS
        (
            SELECT
                ...
                COALESCE(
                    CASE
                        ...
                        THEN (case when p.partner_code is not null then concat('[', p.partner_code, '] ', p.name) else p.name end)
                        ELSE (case when p.partner_code is not null then concat('[', p.partner_code, '] ', p.name) else p.name end)
                    END,
            ...
            INNER JOIN
                account_move_line ml ON a.id = ml.account_id AND ml.date <= %s AND ml.reconciled IS FALSE
        """
        if self.only_posted_moves:
            query_inject_partner += """
            INNER JOIN
        ...
        if self.show_other_currency:
            other_currency_query = """ AND
                p.transaction_currency_id != """ + str(self.env.user.company_id.currency_id.id)
            query_inject_partner += other_currency_query
        query_inject_partner += """
            GROUP BY
                ra.id,
                a.id,
        ...
        sub_query = """
            ...
            INNER JOIN
                account_move_line ml
                    ON ra.account_id = ml.account_id AND ml.reconciled IS FALSE
        """
...
SELECT
    ...
    case when ml.currency_id is not null then ml.currency_id else company.currency_id end AS currency_id
...
INNER JOIN
    account_move_line ml ON ra.account_id = ml.account_id AND ml.reconciled IS FALSE
INNER JOIN
    move_lines ml2
        ON ml.id = ml2.id
...
INNER JOIN
    res_company company ON m.company_id = company.id
        """
        if not only_empty_partner_line:
            query_inject_move_line += """
INNER JOIN
...
--LEFT JOIN res_currency c ON a.currency_id = c.id
...
SET
    ...
    final_amount_residual_currency =
        (
            SELECT
                SUM(rp.final_amount_residual_currency) AS final_amount_residual_currency
            FROM
                report_open_items_qweb_partner rp
            WHERE
                rp.report_account_id = report_open_items_qweb_account.id
