...
class PurchaseInvoiceRegisterReportWizard(models.TransientModel):
    ...
    def action_print_report(self):
        ...
        domain_default = []
        domain_default = [('date_invoice', '>=',  self.date_from), ('date_invoice', '<=',  self.date_to), ('type', 'in', ('in_invoice', 'in_refund')),('state', 'in', ('open', 'paid'))]
        if self.partner_ids:
        ...
        for each in invoice_records:
            ...
            date_due = ""
            if each.date_due:
                date_due = time.strptime(each.date_due, "%Y-%m-%d")
                date_due = time.strftime('%d-%m-%Y', date_due)
            sheet1.write(row, 7, date_due, Style.normal_left())
            ...
            sheet1.write(row, 10, (each.reference and each.reference or ""), Style.normal_left())
            ...
            if each.type == 'in_invoice':
                ...
                sheet1.write(row, 15, each.residual and each.residual or 0.00, Style.normal_num_right_3separator())
                sheet1.write(row, 16, abs(each.amount_untaxed_signed), Style.normal_num_right_3separator())
                sheet1.write(row, 17, abs(each.amount_tax_company_currency), Style.normal_num_right_3separator())
                sheet1.write(row, 18, abs(each.amount_total_company_signed), Style.normal_num_right_3separator())
                sheet1.write(row, 19, abs(each.residual_company_signed), Style.normal_num_right_3separator())
                local_amt += abs(each.amount_untaxed_signed)
                local_tax += abs(each.amount_tax_company_currency)
                local_total += abs(each.amount_total_company_signed)
                local_due += abs(each.residual_company_signed)
            else:
                ...
                sheet1.write(row, 15, each.residual and (-1 * each.residual) or 0.00, Style.normal_num_right_3separator())
                sheet1.write(row, 16, (-1 * abs(each.amount_untaxed_signed)), Style.normal_num_right_3separator())
                sheet1.write(row, 17, (-1 * abs(each.amount_tax_company_currency)), Style.normal_num_right_3separator())
                sheet1.write(row, 18, (-1 * abs(each.amount_total_company_signed)), Style.normal_num_right_3separator())
                sheet1.write(row, 19, (-1 * abs(each.residual_company_signed)), Style.normal_num_right_3separator())
                ref_local_amt += -1 * abs(each.amount_untaxed_signed)
                ref_local_tax += -1 * abs(each.amount_tax_company_currency)
                ref_local_total += -1 * abs(each.amount_total_company_signed)
                ref_local_due += -1 * abs(each.residual_company_signed)
