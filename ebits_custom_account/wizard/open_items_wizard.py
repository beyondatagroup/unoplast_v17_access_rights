...
class OpenItemsReportWizard(models.TransientModel):
    ...
    receivable_accounts_only = fields.Boolean(string="Receivable Accounts Only")
    payable_accounts_only = fields.Boolean(string="Payable Accounts Only")
    partner_ids = fields.Many2many(
        comodel_name='res.partner',
        string='Filter partners',
    )
    has_second_currency = fields.Boolean(string="With Currency")
    ...
    def _prepare_report_open_items(self):
        ...
        return {
            ...
            'has_second_currency': self.has_second_currency,
        }

    def _export(self, xlsx_report=False):
        """Default export is PDF."""
        model = self.env['report_open_items_qweb']
        report = model.with_context(landscape=True).create(self._prepare_report_open_items())
