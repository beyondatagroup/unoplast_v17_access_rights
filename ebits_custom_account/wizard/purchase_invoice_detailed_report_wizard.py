...
class PurchaseInvoiceDetailedReportWizard(models.TransientModel):
    ...
    def action_report(self):
        ...
        for each in invoice_records:
            ...
            sheet1.write(row, 8, (each.origin and each.origin or ""), Style.normal_left())
            sheet1.write(row, 9, (each.reference and each.reference or ""), Style.normal_left())
            ...
            for line in each.invoice_line_ids:
                ...
                if each.type == 'in_invoice':
                    ...
                    sheet1.write(row, 18, abs(line.price_subtotal_signed), Style.normal_num_right_3separator())
                    sheet1.write(row, 19, abs(line.price_subtotal_incl_signed), Style.normal_num_right_3separator())
                    local_inv_amt += abs(line.price_subtotal_signed)
                    ...
                    sheet1.write(row, 16, -1 * line.price_subtotal_incl, Style.normal_num_right_3separator())
                    sheet1.write(row, 17, (line.account_analytic_id and line.account_analytic_id.name or ""), Style.normal_left())
                    sheet1.write(row, 18, -1 * abs(line.price_subtotal_signed), Style.normal_num_right_3separator())
                    sheet1.write(row, 19, -1 * abs(line.price_subtotal_incl_signed), Style.normal_num_right_3separator())
                    local_refund_amt += -1 * abs(line.price_subtotal_signed)
