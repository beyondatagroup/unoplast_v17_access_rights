...
class SaleReturnReportWizard(models.TransientModel):
    _name = 'sale.return.report.wizard'
    _description = 'Sale Return Report Wizard'
    
    date_from = fields.Date(string='From Date', required=True)
    date_to = fields.Date(string='To Date', required=True)
    partner_ids = fields.Many2many('res.partner', 'etc_sale_return_partner', 'sale_return_wizard_id', 'partner_id', string='Customer')
    warehouse_ids = fields.Many2many('stock.warehouse', 'etc_sale_return_warehouse', 'sale_return_wizard_id', 'warehouse_id', string='Warehouse')
    product_ids = fields.Many2many('product.product', 'etc_sale_return_product', 'sale_return_wizard_id', 'product_id', string='Product')
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(SaleReturnReportWizard, self).fields_view_get(
    ...
    def action_report(self):
        ...
        report_name = "Sales Return Status"
        filters = ""
        customer_sql = """ """
        warehouse_sql = """ """
        product_sql = """ """
        ...
        if self.state:
            # date_from = time.strptime(from_date,"%d-%m-%Y")
            date_from = self.date_from.strftime('%d-%m-%Y')
            # date_to = time.strptime(to_date,"%d-%m-%Y")
            date_to = self.date_to.strftime('%d-%m-%Y')
            print("\n\n\n\n=======date_to==========",date_to)
            date_sql += (
                " and ((sm.date_expected at time zone %s)::timestamp::date) >= '" 
                + str(self.date_from) 
                + "' and ((sm.date_expected at time zone %s)::timestamp::date) <= '" 
                + str(self.date_to) 
                + "'"
            )
            filters += " Return Date From : " + str(self.date_from) + ' To ' + str(self.date_to)
        ...
        if self.product_ids:
            ...
            print("\n\n\n\n=======customer_sql + warehouse_sql + product_sql + status_sql + date_sql",customer_sql + warehouse_sql + product_sql + status_sql + date_sql)
        sale_return_sql = """select 
                            ...
                            where spt.code = 'incoming'  """+ customer_sql + warehouse_sql + product_sql + status_sql + date_sql + """group by sp.name,
                            	rp.name,
	                            sm.date_expected,
	                            sp.date_done,
	                            sw.name,
        ...
        title2 = filters
        if self.state:
            title = report_name +' ( Sales Return Date From ' + date_from + ' To ' +  date_to + ' )'
        ...
        sheet1.write(r3, 3, "Customer", Style.contentTextBold(r2,'black','white'))
        ...
        sheet1.write(r3, 6, "Sale order No", Style.contentTextBold(r2,'black','white'))
        ...
        return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'sale.return.report.wizard',
                'res_id': self.id,
