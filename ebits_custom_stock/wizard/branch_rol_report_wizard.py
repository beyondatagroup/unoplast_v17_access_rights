...
class BranchRolReportWizard(models.TransientModel):
    _name = 'branch.rol.report.wizard'
    _description = 'Branch ROL Report Wizard'
    
    product_ids = fields.Many2many('product.product', 'etc_branch_rol_report_product', 'branch_rol_report_wizard_id', 'product_id', string='Product')
    warehouse_ids = fields.Many2many('stock.warehouse', 'etc_branch_rol_report_warehouse', 'branch_rol_report_wizard_id', 'warehouse_id', string='Branch')
    view_location_ids = fields.Many2many('stock.location', 'etc_branch_rol_report_view_location', 'branch_rol_report_wizard_id', 'view_location_id', string='View Location')
    location_ids = fields.Many2many('stock.location', 'etc_branch_rol_report_location', 'branch_rol_report_wizard_id', 'location_id', string='Location')
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(BranchRolReportWizard, self).fields_view_get(
...
#            self.view_location_ids = False
    ...
    def action_report(self):
        report_name = "Branch Reordering Level Report"
        filters = ""
        product_sql = """ """
        location_sql = """ """
        warehouse_sql = """ """
        ...
        if warehouse_ids:
            ...
            if len(warehouse_ids) > 1:
                warehouse_sql += "and sb_op.warehouse_id in "+ str(tuple(warehouse_ids))
            else:
                warehouse_sql += "and sb_op.warehouse_id in ("+ str(warehouse_ids[0]) + ")"
            filters += "  Branch: " + warehouse_str
        if self.product_ids:
        ...
        if self.location_ids:
            ...
            if len(location_ids) > 1:
                location_sql += "and sb_op.location_id in "+ str(tuple(location_ids))
            else:
                location_sql += "and sb_op.location_id in ("+ str(location_ids[0]) + ")"
        ...
        rol_sql = """select * from (select 
	                            ...
	                            (case when sb_op.product_min_qty is not null then sb_op.product_min_qty else 0.00 end) as min_qty,
	                            (case when sb_op.product_max_qty  is not null then sb_op.product_max_qty else 0.00 end) as max_qty,
                            ...
                            from stock_branch_orderpoint sb_op 
	                            left join  product_product pp on (pp.id = sb_op.product_id) 
	                            left join product_template pt on (pt.id = pp.product_tmpl_id)
	                            left join uom_uom pu on (pu.id = pt.uom_id)
	                            left join product_category pc on (pc.id = pt.categ_id)
	                            left join stock_warehouse sw on (sw.id = sb_op.warehouse_id)
	                            left join stock_location sl on (sl.id = sb_op.location_id)
                            ...
                            sw.name,
                            sl.name, 
                            sb_op.product_min_qty,
                            sb_op.product_max_qty 
        ...
        sheet1.write(r4, 4, "Branch", Style.contentTextBold(r2,'black','white'))
        ...
        if rol_data:
            for line in rol_data:
                ...
                # quality_location_qty field not available
                ...
                total_qty = (line['max_qty'] - abs((line['stock_location_qty'] + line['st_request_qty'] + line['st_issue_qty'] + line['st_receipt_qty'] + line['po_qty'] + line['grn_qty']  + line['mo_qty']) - (line['do_qty'] + line['st_out_issue_qty'])))
        ...
        return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'branch.rol.report.wizard',
                'res_id': self.id,
